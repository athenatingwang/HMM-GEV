...
while(kk <= 500)
{
  ...
  tryCatch({
    y1 <- nlm(hmm.evd,p=param0,data=R1hrmax[55000:100000],nk=nk,iterlim = 10000,steptol = 1e-3,print.level=1)
    y <- nlm(hmm.evd,p=y1$estimate,data=R1hrmax,nk=nk,iterlim = 10000,steptol = 1e-4,hessian =TRUE,print.level=2)
    ...
    if (abs(nLL)<21020){ # use 27228.2 for 2S # use 21287.23 for 4S, 21167.39 for 5S # 21020 for 6S and 7S
      catched = TRUE
      eval(parse(text=paste('HMM',nk,'est = HMMest',sep="")))
      eval(parse(text=paste('save(HMM',nk,'est, file="HMMgevHr',nk,'est',kk,'.image")',sep='')))
    }
  }, error=function(e){})
}
